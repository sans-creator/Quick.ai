  ...
  const [object, setObject] = useState('')
    ...
    const onSubmitHandler = async (e) => {
        ...
        if(object.split(' ').length > 1) {
         return toast('Please enter only a single object name to remove');
        }
 ...
 return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <p className='mt-6 text-sm font-medium'>Describe object name to remove</p>
       ...
       rounded-md border border-gray-300' placeholder='e.g., a dog, a car, Only single object name' required/>
    
      <button disabled={loading} className='w-full flex justify-center items-center gap-2 mt-6 bg-gradient-to-r from-[#417DF6] to-[#8E37EB] text-white px-4 py-2 text-sm rounded-lg cursor-pointer '>
         {
